...
export class CatchPage implements OnInit {
  ...
  private atacarSalvaje() {
    ...
    if (this.pokemonBatalla.hp > 0) {
      if (this.pokeSalvaje.hp > 0) {
        ...
        this.barraHP_wild = this.lvup.calcularBarraHpPokemon(this.barraHP_wild, this.pokeSalvaje.hp, this.pokeSalvaje.hp_max);
      }
    }
  }
  ...
  private captura(poke: string, ball: string): boolean {
    ...
    this.pokeSalvaje.ball = 'nada'; let numstate = 0;

    if (this.pokeSalvaje.state === '') { this.pokeSalvaje.state = 'nada'; }
    console.log(poke);
    ...
    switch (ball.toLowerCase().substring(0, 4)) {
      case 'poke':
        ...
        ratioBall = 1.5;
        numstate = this.getNumstate(this.pokeSalvaje.state);
        break;
      case 'supe':
        ratioPoke = 150;
        rand = (Math.random() * (0 - ratioPoke + 1) + ratioPoke);
    ...
    a = (((3 * this.pokeSalvaje.hp_max - 1 * this.pokeSalvaje.hp) * ratioCaptura * ratioBall) / (1 * this.pokeSalvaje.hp_max)) + numstate + bonus;
    console.log(`(((3 * ${this.pokeSalvaje.hp_max} - 1 * ${this.pokeSalvaje.hp}) * ${ratioCaptura} * ${ratioBall}) / (1 * ${this.pokeSalvaje.hp_max})) + ${numstate} + ${bonus};`);
    console.log(a);
  ...
  private calcular_variables(pokeAtaca: PokemonInterface, pokedefense: PokemonInterface) {
    ...
    // Calculo de daño
    aux = ((0.2 * pokeAtaca.level! + 1) * pokeAtaca.attack * this.potencia) / (25 * pokedefense.defense) + 2;
  ...
  async startAnimation() {
    ...
    this.throwBallAnimation = this.animationCtrl.create().addElement(bal!).duration(4000).iterations(1).keyframes([
      ...
      { offset: 0.5, top: 'calc(30% - 100px)', transform: 'scale(0.9)' },
      { offset: 1, top: 'calc(38% - 100px)', left: 'calc(85% - 100px)', transform: 'scale(1)' }
    ]);
